...
function validateInput(input) {
    ...
    return input.length > 0 && input.length <= 100;
        ...
        console.log(`   - Vector store: ${stats.totalVectors} vectors, ${stats.embeddingDimension} dimensions`);
        const storageStats = storage.getStats();
        console.log(`   - Files in storage: ${storageStats.fileCount} files, ${storageStats.totalChunks} chunks`);
